...
<div class="kegiatan">
    <script>
        ...
        $(function(){
            ...
            $("#formnew").submit(function(){        
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url("demografi/antrian_fisioterapi_save") ?>/',
                    cache: false,
                    data: $('#formnew').serialize()+'&tgl_lahir='+Dtgl_lahir,
                    dataType :'json',
                    success: function(data) {
                       if(data.status == true){
                            custom_message('Berhasil','Transaksi Berhasil');
                            $('#simpan').hide();
                            cetak_antrian(data.id_pelayanan_kunjungan);
                       }else{
        ...
        function cetak_antrian(id_pk){
            var dWidth = $(window).width();
            var dHeight= $(window).height();
            var x = screen.width/2 - dWidth/2;
            var y = screen.height/2 - dHeight/2;
            window.open('<?= base_url() ?>pendaftaran/cetak_no_antri/'+id_pk, 'Cetak Nomor Antri Pendaftaran','width='+dWidth+', height='+dHeight+', left='+x+',top='+y);
        }
    ...
    <div class="data-input">
        ...
        <table width="100%" class="inputan">Pasien</legend>           
            ...
            <?= form_hidden('jenis','langsung') ?>
            
            
            <tr><td>Alamat Jalan*:</td><td><?= form_textarea('alamat','','id=alamat class="standar enter"')?>
            <tr><td>Kelurahan:</td><td><?= form_input('kelurahan', '', 'id=kelurahan size=30 class="input-text enter"') ?> 
            <?= form_hidden('id_kelurahan') ?>    
            <tr><td></td><td><span class="label" id="addr"></span> 
            <tr><td>Jenis Kelamin:</td><td><?= form_dropdown('kelamin', $kelamin, '', 'id = kelamin class="standar enter"') ?>        
            ...
            <tr><td>Telepon:</td><td><?= form_input('tlpn', null, 'id=telp class="input-text angka enter" size=30') ?>
            <tr><td>Golongan Darah</td><td><?= form_dropdown('gol_darah', $darah,null,'id=darah_gol class="standar enter"') ?>        
        </table>
        <table width="100%" class="inputan">Antrian</legend>
            <tr><td>Tanggal</td><td><span class="label" id="tanggal"><?= date('d/m/Y') ?></span><?= form_hidden('tgl_layan',date('Y-m-d')) ?>
            <tr><td>Layanan</td><td><span class="label">Fisioterapist</span><?= form_hidden('id_jurusan', $id_jurusan) ?>
            <tr><td>No. Antrian</td><td><span class="label" id="antri"><?= $antri?><?= form_hidden('antrian', $antri)?></span>
            <tr><td></td><td>
